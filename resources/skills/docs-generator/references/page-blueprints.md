...
- **API:** Method/path or interface, authentication, request/response contract, errors, pagination and idempotency where implemented; link canonical specifications. For a service with authenticated HTTP endpoints aimed at testers/integrators, see [api-contract-template.md](api-contract-template.md) for a canonical page shape.
