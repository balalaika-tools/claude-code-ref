...
- Classify the task before choosing paths: targeted update, full generation, or clean rebuild. Treat a request to create a repository manual when no coherent documentation index exists as full generation. In full generation and clean rebuild, choose one coherent page map from the blueprint and repository evidence; do not let files encountered during discovery silently become additional destinations.
...
Maintain a working evidence map: topic → relevant files/symbols → observed behavior → uncertainty → destination page. For full generation or a clean rebuild, derive a page manifest from it before writing. Every planned Markdown file should have a reader task and a place in the navigation; record the concrete reason for any top-level page outside the preferred blueprint. These can remain working notes; do not generate a second catalog that duplicates the manual.
...
## 3. Adapt the preferred blueprint

Use [page-blueprints.md](references/page-blueprints.md) when choosing pages or writing a substantial new section. For new documentation or a full rebuild, use its blueprint as the preferred starting structure after discovering the repository; honor an explicitly agreed structure. Preserve folder names, page names and topic organization wherever they fit. Omit inapplicable folders and pages, combine thin related pages, and add pages or folders for substantial project-specific needs. Each departure should serve a concrete reader need or repository characteristic. Libraries, CLIs, mobile apps and mixed monorepos may need different coverage. For targeted updates, preserve established paths unless restructuring was requested.

Keep the `docs/` root intentional. Under the preferred blueprint it contains the documentation index and, when warranted, the overview; place procedures under `guides/`, exact contracts under `reference/`, operational material under `operations/`, architecture under `architecture/`, and deployable-specific material under `services/`. Create another root-level page only for an established repository convention, an explicitly agreed stable public path, or content that demonstrably does not fit those categories.

Do not create compatibility wrappers or duplicate pages solely because a test, source file, or obsolete internal link hard-codes an old documentation filename. A hard-coded path is a consumer mismatch, not by itself a documentation-architecture requirement. During a rebuild, repair documentation-owned links and report non-documentation consumers that require a follow-up change, including the old-to-new path mapping. Preserve a compatibility path only when the user requests it or evidence establishes a real externally consumed URL/versioning contract; keep the exception minimal and report it in the handoff.

Organize documentation around reader tasks and conceptual boundaries; it need not mirror the source-code directory tree. Distinguish inapplicable topics from missing knowledge about applicable workflows: retain material gaps near the affected content instead of omitting the topic. Required coverage means a usable reader workflow, not merely a heading; avoid empty pages and repeated `Not applicable` entries.
