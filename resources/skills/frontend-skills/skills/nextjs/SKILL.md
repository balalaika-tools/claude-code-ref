---
name: nextjs
description: Current Next.js 16 App Router guidance for caching, Server Components, routing, Server Actions, framework contracts, security boundaries, assets, debugging, and deployment. Use for Next.js 16.x code. Generic React 19 patterns are covered by the react skill.
---
...
## Framework Contracts and Operations

Read only the guide that matches the task:

- For App Router file conventions, async request APIs, RSC boundaries, Route Handlers, or runtime selection, read [app-router-contracts.md](references/app-router-contracts.md).
- For authentication and authorization in Server Actions, Route Handlers, Proxy, or data access, read [security-server-entrypoints.md](references/security-server-entrypoints.md).
- For `next/image`, `next/font`, or `next/script`, read [asset-delivery.md](references/asset-delivery.md).
- For standalone output, containers, multiple instances, cache coordination, or rolling deployments, read [self-hosting.md](references/self-hosting.md).
- For Next.js DevTools MCP, runtime inspection, route type generation, or targeted build diagnostics, read [runtime-debugging.md](references/runtime-debugging.md).
