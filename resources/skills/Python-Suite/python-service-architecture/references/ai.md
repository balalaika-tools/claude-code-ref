...
Classify middleware by purpose, not by the framework hook it uses. Middleware
that changes agent behavior or policy—attempt budgets, timeout, fallback,
summarization, token limits, or tool execution policy—belongs to the owning
`genai/` task or demonstrated shared GenAI capability. A callback, middleware,
usage parser, or invocation wrapper whose only effect is tracing, metrics,
logging, or correlation belongs in the service's top-level `observability/`
package by default, even when it imports LangChain.

Keep generic SDK/provider construction in a separate module such as
`observability/tracing.py`; do not mix framework adapters into that module and do
not make `observability/__init__.py` eagerly import optional GenAI dependencies.
Start flat with a precise module such as `observability/genai.py` or
`observability/langchain.py`. Introduce a nested GenAI-observability package only
when a large collection of independently changing adapters, distinct lifecycle
or test setup, or demonstrated import pressure makes the split valuable. One
callback plus one tool middleware does not meet that threshold. Observability
must not import task agents or prompts.
