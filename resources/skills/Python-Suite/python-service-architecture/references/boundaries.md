...
Telemetry ownership does not change because a framework requires a callback or
middleware hook. Keep LangChain callbacks, tool-tracing middleware, provider
usage parsers, and agent-span wrappers in precise modules such as
`observability/genai.py` or `observability/langchain.py` by default. Keep generic
provider setup in a separate `observability/tracing.py`; it must not import those
framework adapters. Introduce a nested `genai/observability/` or equivalent only
after a large, independently changing instrumentation surface, distinct lifecycle
or test setup, or demonstrated import/package pressure justifies the extra boundary.
The mere presence of a LangChain import does not justify it.
