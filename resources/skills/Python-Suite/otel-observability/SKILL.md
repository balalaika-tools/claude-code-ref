...
7. **Separate SDK setup from framework instrumentation by module, not automatically by package tree.** Keep telemetry-only callbacks, middleware, usage adapters, and agent wrappers in `observability/` by default, but never in the `TracerProvider` module.
   Start with `observability/genai.py` or `observability/langchain.py`; nest only for a large independently changing collection, distinct lifecycle/test setup, or demonstrated import pressure.
   One callback plus one middleware stays flat.
