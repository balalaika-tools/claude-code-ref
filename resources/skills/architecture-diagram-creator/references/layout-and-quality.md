...
For collision fixes, first consider a different card-face anchor, a nearby routing lane, deliberate heading line breaks, or a local card adjustment. When a connection creates crowding or a long detour, consider whether small shifts of neighboring nodes—even those not involved in the connection—would improve the composition. Prefer the arrangement that makes all affected connections clearer overall while preserving the main flow and architectural boundaries. Resize a boundary only when its content, readable heading or necessary routing space needs it. Do not stretch a frame simply to move its heading out of a connector's way. Avoid routes running along boundary borders, where communication can be confused with containment.
