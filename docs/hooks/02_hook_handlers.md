...
> **Key insight**: Choosing a handler type is a determinism trade-off, not a feature preference — reach for `prompt`/`agent` only when a script genuinely cannot express the check, since every model-backed handler adds latency and non-determinism to something that fires on every matching event.
