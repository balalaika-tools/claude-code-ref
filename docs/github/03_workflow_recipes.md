...
> **Key insight**: Model choice and permission scope should track each other across these recipes — cheap, low-risk workflows like triage use `haiku` with read-mostly permissions, while high-stakes ones like security review pair `opus` with write access, so risk and capability rise together rather than independently.

---

**Back to**: [GitHub Index](index.md)
