...
> **Key insight**: Dynamic `` !`command` `` blocks execute once before Claude ever sees the prompt, so interpolating user-controlled arguments into them is a shell-injection risk that Claude cannot catch or refuse in the moment.
