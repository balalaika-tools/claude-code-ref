...
> **Key insight**: Migrating a workflow from a command file to a same-named skill silently shadows the old command — the skill wins with no warning, so check for name collisions rather than assuming the command still fires.
