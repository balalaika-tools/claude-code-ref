...
> **Key insight**: Because permission arrays merge across scopes and `deny` always wins regardless of which scope set it, `deny` is the only reliable way to guarantee a tool stays blocked even when a lower-priority scope tries to allow it.
