...
> **Key insight**: Committing a plugin at project scope only commits a pointer — each teammate still fetches and caches the actual plugin code locally, so trusting the repo is not the same as vetting what gets run.
