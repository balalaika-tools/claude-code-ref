...
> **Key insight**: A marketplace is not just a discovery index — removing it cascades to uninstall every plugin sourced from it, so treat `marketplace remove` as a destructive operation, not a simple cleanup.
